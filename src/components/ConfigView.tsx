...
import { parseConfigSettings, buildConfigContent, detectTriggerSymbol, applyTriggerSymbolToAll, extractMatches, extractGlobalVars, buildMatchFileContent } from '../utils/yaml'
import { NewlineMode, getDefaultNewlineMode, setDefaultNewlineMode } from '../utils/prefs'
...
export default function ConfigView({ configInfo, showToast, onHeaderActionsChange }: Props) {
  ...
  const loadTriggerSymbol = useCallback(async () => {
    try {
      ...
      const symbol = detectTriggerSymbol(allMatches)
      ...
      if (applyTo === 'all') {
        ...
        if (files.success && files.files) {
          for (const file of files.files) {
            const fileResult = await window.espansoAPI.readMatchFile(file.name)
            if (fileResult.success && fileResult.content) {
              const matches = extractMatches(fileResult.content)
              const fileGlobalVars = extractGlobalVars(fileResult.content)
              const updatedMatches = applyTriggerSymbolToAll(matches, newSymbol)
              const newContent = buildMatchFileContent(updatedMatches, fileGlobalVars, `# ${file.name.replace(/\.(yml|yaml)$/, '')}\n# Managed by Espanso GUI\n`)
    ...
    <div>
        ...
        <div>
          <div className="settings-row">
            <div>
              ...
              <div className="settings-desc">Clipboard backend to use</div>
            </div>
            <select
              className="form-select"
              style={{ width: 200 }}
              value={settings.backend || 'Clipboard'}
              onChange={e => updateSetting('backend', e.target.value)}
            >
              <option value="Clipboard">Clipboard</option>
              <option value="ClipboardWin32">Clipboard Win32</option>
              <option value="ClipboardX11">Clipboard X11</option>
              <option value="ClipboardWayland">Clipboard Wayland</option>
              <option value="ClipboardMac">Clipboard Mac</option>
            </select>
          </div>
          ...
          <div className="settings-row">
            <div>
              ...
              <div className="settings-desc">Max characters for clipboard paste (0 = always use clipboard)</div>
            </div>
            <input
              className="form-input"
              type="number"
              style={{ width: 120 }}
              value={settings.clipboard_threshold || 0}
              onChange={e => updateSetting('clipboard_threshold', parseInt(e.target.value) || 0)}
            />
          </div>
          ...
          <div className="settings-row">
            <div>
              ...
              <div className="settings-desc">Delay in milliseconds before pasting</div>
            </div>
            <input
              className="form-input"
              type="number"
              style={{ width: 120 }}
              value={settings.pre_paste_delay || 0}
              onChange={e => updateSetting('pre_paste_delay', parseInt(e.target.value) || 0)}
            />
          </div>
          ...
          <div className="settings-row">
            ...
            >
              {TOGGLE_KEY_OPTIONS.map(opt => (
                <option key={opt} value={opt}>{opt === '' ? 'Default (double-tap ALT)' : opt}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      {renderSection('advanced', 'Advanced Settings', (
        <div>
          <div className="settings-row">
            <div>
              <div className="settings-label">Match Files</div>
              <div className="settings-desc">Additional match files to include (comma separated)</div>
            </div>
            <input
              className="form-input font-mono"
              style={{ width: 300 }}
              value={(settings.match || []).join(', ')}
              onChange={e => updateSetting('match', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
            />
          </div>
          ...
          <div className="settings-row">
            <div>
              ...
              <div className="settings-desc">Additional config files to include</div>
            </div>
            <input
              className="form-input font-mono"
              style={{ width: 300 }}
