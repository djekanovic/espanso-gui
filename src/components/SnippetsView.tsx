...
import { extractMatches, extractGlobalVars, getMatchLabel, getMatchTriggers, getMatchReplacement, getMatchType, triggerExists, buildMatchFileContent } from '../utils/yaml'
  ...
  const loadMatches = useCallback(async (filename: string) => {
    try {
      ...
      if (result.success && result.content) {
        const vars = extractGlobalVars(result.content)
        setMatches(extractMatches(result.content))
        setGlobalVars(vars)
        lastSavedGlobalVars.current = vars
