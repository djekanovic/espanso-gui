    ...
    const loadStats = async () => {
      try {
        ...
        if (files.success && files.files) {
          ...
          // Extensions & forms = entries under global_vars (anything after
          // the "global_vars:" marker). Rough but dashboard-appropriate.
          const vars = contents.reduce((total, content) => {
            if (!content.success || !content.content) return total
            const section = content.content.split('global_vars:')[1] || ''
            return total + (section.match(/^\s*- name:/gm) || []).length
          }, 0)
          setVarCount(vars)
        }
